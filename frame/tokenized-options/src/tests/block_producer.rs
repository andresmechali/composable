...
use proptest::prelude::{prop, prop_compose, Strategy};
use sp_runtime::{
	traits::{AtLeast32Bit, Bounded, CheckedAdd, One, Saturating},
	DispatchResult,
};
...
impl<C: BlocksConfig> BlockData<C> {
	prop_compose! {
		fn generate(
			interval_range: Range<u32>,
			extrinsic_count_range: Range<usize>,
		)(
			interval in interval_range,
			extrinsic_count in extrinsic_count_range
		) -> Self {
			BlockData { interval: MomentOf::<C>::from(interval), extrinsic_count }
		}
	}
}
...
{
	pub fn generate(
		block_count_range: Range<usize>,
		interval_range: Range<u32>,
		extrinsic_count_range: Range<usize>,
		extrinsic_strategy: impl Strategy<Value = C::Extrinsic> + Clone,
	) -> impl Strategy<Value = Self> {
		prop::collection::vec(
			BlockData::generate(interval_range, extrinsic_count_range),
			block_count_range,
		)
